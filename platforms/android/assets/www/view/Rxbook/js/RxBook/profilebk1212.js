var profile = {
    ...
    init: function () {
        ...
        $('body').append('<div class="loader-div" style="display: none"><img class="loader" src="images/RxBook/animation.gif" alt=""/></div>');
    },

    getUserImportantDate: function() {
        var _this = this;
        Services.getUserImportantDate(curUserId, function (data) {
            _this.bindUserImportantDate(data);
        }, function (e) { });
    },
    bindUserImportantDate: function (data) {
        $('#empDoa p').show();
        $('#empDob p').show();
        $('#empDob input').hide();
        $('#empDoa input').hide();
        ...
        if (data && data.length > 0) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (profile.isAdmin) {
                    ...
                    var $delBtn = $('<a href="#" class="fa fa-trash" title="delete"></a>');
            ...
            content.html('<ul></ul>').data('fEduObj', new Array());
        }
    ...
    editEducations: function (data) {
        ...
        html += '<div class="field" id="eduname"><b>Concentration</b><input type="text" id="education_name" name="education_name" value="' + (curData.Education_Name ? curData.Education_Name : '') + '" /></div>';
        html += '<div class="field" id="timeperiod"><b>Time Period</b>';
    ...
    bindUserWorkDetails: function (data) {
        ...
        if (data && data.length > 0) {
            ...
            content.html('<ul></ul>').data('fWorkObj', new Array());
}
    },
    ...
    editWork: function (data) {
        ...
        html += '<div class="field" id="workperiod"><b>Time Period</b>';
    ...
    editInterestDetails: function () {
        ...
        $('.add-list.add-interest').unbind('click').bind('click', function () {
            ...
            $('.add-interest, .edit-interest').hide();
            $('.contents.intrest').prepend('<div class="inner-edit add" style="padding: 5px 10px">' +
                ...
                '<a href="#" class="cancel" title="Cancel">Cancel</a></div></form></div>');
            ...
            $('.inner-edit.add .save').unbind('click').bind('click', function () {
                ...
                if (prevObj == undefined || prevObj == null) {
                    ...
                    for (var i = 0; i < prevObj.length; i++) {
                        ...
                        if (cObj.Interest_Name.toLowerCase().toString() != iname.toLowerCase().toString()) {
                            newObj.Interest_Name = iname;
                            newObj.Interest_Id = null;
                            interestAry.push(newObj);
                        }
    ...
    bindLifeEvents: function (data) {
        ...
        if (data && data.length > 0) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (profile.isAdmin)
                    html += '<div class="edit-actions"><a href="#" class="fa fa-pencil edit" title="edit"></a><a href="#" class="fa fa-trash delete" title="delete"></a></div>';
                $li.html(html);
            ...
            content.html('<ul></ul>').data('feventsObj', new Array());
        }
    },
        ...
        Services.getKWUserInfo(curUserId, function (data) {
            ...
            if (data && data.length > 0) {
                ...
                $('.doctor-img img').attr('src', doctor.Profile_Photo_BLOB_URL);
                    ...
                    $('.doctor-img img').attr('src', 'http://docporch.blob.core.windows.net/user/p3300 008.png');
                }
