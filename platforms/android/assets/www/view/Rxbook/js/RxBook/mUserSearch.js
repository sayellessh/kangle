var friends = {
    ...
    subscribedOriginalFriends: null,
    init: function () {
        friends.getFriends();
    },
    getFriends: function (onSuccess) {
        ...
        Services.getTopics(Services.defaults.userId, function (data) {
        	...
        	app.bindUserErrorFeed($(".search-view ul"), networkProblemError);
        });
    },

    searchFriends: function () {
        $('.search-friend').show();
        $('input.search-text').bind('keypress', function (e) {
            if (e.keyCode == 13) {
                var val = $('input.search-text').val().trim();
                searchObject.inputArray = friends.subscribedFriends;
                searchObject.searchByField = 'displayName';
                searchObject.init(val, function (array) {
                    friends.bindSearchFriends(array);
                });
            }
        });
        $('span.search-actn').unbind('click').bind('click', function () {
            var val = $('input.search-text').val().trim();
            searchObject.inputArray = friends.subscribedFriends;
            searchObject.searchByField = 'displayName';
            searchObject.init(val, function (array) {
        ...
        $('span.search-clear').unbind('click').bind('click', function () {
            friends.bindFriends(friends.subscribedOriginalFriends);
        });
    },
    ...
    bindFriends: function (data) {
        ...
        if (data != null && data.length > 0) {
            friends.subscribedOriginalFriends = data;
            friends.subscribedFriends = new Array();
            for (var i = 0; i < data.length; i++) {
                ...
                if (subscriber != null && subscriber.isActive && !subscriber.isRemoved && friend != null && friend.isActive && !friend.isRemoved) {
                    ...
                    friends.subscribedFriends.push(subscriber);
                }
            }
        } else {
            friendsList.html('<li><div><span class="friend-name">No friends found</span></div></li>');
        }
        console.log(friends.subscribedFriends);
        friends.searchFriends();
        friends.bindActions(false);
    },
    bindSearchFriends: function(data) {
        var friendsList = $('.search-view ul');
        friendsList.empty();
        if (data != null && data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                var curFrnd = data[i];
                var $li = $('<li></li>');
                var html = '<img src=" ' + (curFrnd.profilePicURL != null ? curFrnd.profilePicURL : Services.defaults.defaultThumbnail) + '" alt="">';
                html += '<div>';
                html += '<div class="friend-name">' + curFrnd.displayName + '</div>';
                html += '<div class="friend-action"><a href="#" class="btn btn-profile">Profile</a>' +
                    '<a href="#" class="btn btn-chat">Chat</a>' +
                    '<a href="#" class="btn btn-unfriend">UnFriend</a></div>';
                html += '</div>';
        ...
        friends.bindActions(true);
    },
    bindActions: function (isSearch) {
        ...
        $('.friend-detail ul li a.btn-unfriend').bind('click', function () {
            var fObj = $(this).parents('li').eq(0);
            fObj = fObj.data('friendObj');
            var fVal = window.confirm('Do you want to unfriend ' + (isSearch == true ? fObj.displayName : fObj.Display_Name));
            if (fVal) {
                if (isSearch) {
                    Services.removeTopic(Services.defaults.userId, fObj.topicId, fObj.userId, function (data) {
                        friends.getFriends();
    ...
    removeFriend: function (fObj) {
        var friend = app.getFriendFromSubscriptions(fObj.subscriptions);
...
var userSearch = {
    ...
    searchFriends: function (mEl) {
        $(mEl + ' .search-field a').bind('click', function () {
            var val = $(mEl + ' #friend-string').val().trim();
    ...
    createGroup: function (el) {
        ...
        $(el + ' .search-field a').unbind('click').bind('click', function () {
            ...
            if (val == '' || val == undefined || val == null) {
                alert('Group cannot create with empty name');
                return false;
            }
    ...
    getFriendsForPrivateGroup: function (grpObj) {
        ...
        Services.getTopics(Services.defaults.userId, function (friends) {
            ...
            for (var i = 0; i < friends.length; i++) {
                var curFrnd = friends[i];
                if (uniques[curFrnd.Subscription_User_Id] == null && curFrnd.Is_Active) {
                	uniques[curFrnd.Subscription_User_Id] = curFrnd;
                    var existingUser = userSearch.checkIfUserSubscribed(curFrnd, grpObj.subscriptions);
                    var $el = $('<li></li>');
                    var userPic = (curFrnd.User_Profile_Pic!=null?curFrnd.User_Profile_Pic:Services.defaults.defaultThumbnail);
                    $el.append('<img alt="" src="' + (userPic) + '">');
        ...
        for (var i = 0; i <= existUsers.length - 1; i++) {
            if (user.Subscription_User_Id == existUsers[i].userId) {
                if (existUsers[i].isActive == true) {
                    return 'ACTIVE';
                } else if (existUsers[i].isRemoved == true) {
                        return false;
                } else {
                	//alert(existUsers[i].isActive);
                	return 'INACTIVE';
                }
            }
        }
