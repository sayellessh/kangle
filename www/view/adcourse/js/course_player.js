...
Player.prototype.createFrame = function (asset) {
    ...
    if(ext == 'pdf') {
    	...
    	courseFile.start(_this.options.course.Course_ID, asset, function(fileUrl){
    		...
    		_this.analyticsInsert(asset);    		
            var ref = window.open(fileUrl, '_blank', 'location=yes');
            //cordova.plugins.fileOpener2.open(fileUrl,'application/doc');
    	});
        ...
        courseFile.start(_this.options.course.Course_ID, asset, function(fileUrl){
             ...
             _this.analyticsInsert(asset);
             var ref = window.open(fileUrl, '_blank', 'location=yes');
             //cordova.plugins.fileOpener2.open(fileUrl,'application/xls');
         });
    } else if (ext == 'ppt' || ext == 'pptx') {
        courseFile.start(_this.options.course.Course_ID, asset, function(fileUrl){
            app.hideLoading();
            _this.analyticsInsert(asset);
            cordova.plugins.fileOpener2.open(fileUrl,'application/ppt');
        });
        ...
        courseFile.start(_this.options.course.Course_ID, asset, function(fileUrl){
            app.hideLoading();
            _this.analyticsInsert(asset);
            $("#myIframe").show().attr('src', fileUrl);
        });
    } else if (ext == 'swf') {
    	app.hideLoading();
    	_this.analyticsInsert(asset);
