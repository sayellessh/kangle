        ...
        contents: [
            {
		        ...
		        displaytitle: "Register for training",
		        iconclass: 'arrow-register',
		        onclick: function () {
		            alert('Register for training');
		        },
		        isVisible: true
		    }, {
		        isSeparator: true
...
ArrowPopup.prototype.init = function() {
	...
	if (_this.type == _this.types[0]) {
	    ...
	    if (_this.contents != null && _this.contents.length > 0) {
	        for (var i = 0; i <= _this.contents.length - 1; i++) {
	            ...
	            $acceptBtn.bind('click', function (e) {
	                if ($(this).parents('li').length > 0) {
	                    if (_this.onAccept) _this.onAccept($(this).parents('li').data('actionMsg'), $(this));
	                }
	            });
	            var $rejectBtn = $('<a class="fa fa-close notf-reject-btn"></a>');
	            $rejectBtn.bind('click', function (e) {
	                if ($(this).parents('li').length > 0) {
	                    if (_this.onReject) _this.onReject($(this).parents('li').data('actionMsg'), $(this));
	                }
	            });
	        ...
	        // empty feeds
	    	divobj.find('.arrow-container .arrow-content ul').append('<div class="ap-empty">No new request</div>');
	    ...
	    $.each(this.contents, function (ind, obj) {
	        ...
	        if (obj != null && obj.isSeparator) {
	            ...
	            var checkboxLab = '';
	            if (_this.type === _this.types[3])
	                checkboxLab = '<input type="checkbox" />&nbsp;';
	            var lielem = $('<li id="' + arrowLiId + '">' + checkboxLab + '<a href="' + (obj.href == undefined ? "#" : obj.href) + '">' + obj.displaytitle + '</a></li>');
