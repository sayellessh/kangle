var meeting = {
    ...
    getMeetingMaterial: function(url) {
        Services.checkMeetingMaterial(url, function (status) {
            if (status == 0) {
                //alert('There is a file in process list. Please wait until the old file process completed');
                //return false;
                $('#show-files').show();
            } else if (status == 1) {
        ...
        Services.getModeratorMeetings(common.defaults.userId, function (data) {
            ...
            if (data && data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    var meets = data[i];
                    if (meets.Meeting_Status == 0 || meets.Meeting_Status == 1) {
                        if (meeting.checkMeetingIsInTime(meets.End_DateTime)) {
                            newData.push(meets);
                        }
                    }
                }
            ...
            $('#meeting-list').unbind('change').bind('change', function () {
                meeting.getMeetingMaterial($(this).val());
                return false;
            });
        }, function (err) {
            console.log(err);
    ...
    checkMeetingIsInTime: function (meetingEndDateStr) {
        console.log(meetingEndDateStr);
        var meetingEndDate = meeting.getNewDate(meetingEndDateStr), curDate = new Date();
        if (meetingEndDate.getTime() >= curDate.getTime())
            return true;
        return false;
    },
    getNewDate: function (date) {
        var gDate = new Date(date), dateAry = date.split(' ');
        /*dateAry = dateAry[0].split('/');
        var dte = dateAry[0], mnt = dateAry[1], year = dateAry[2];
        gDate.setDate(dte);
        gDate.setMonth(mnt-1);
